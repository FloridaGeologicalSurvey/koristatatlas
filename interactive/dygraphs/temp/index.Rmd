---
title: "FGS 2D-ACM Temperature dygraphs"
...
sql <- "SELECT * FROM rfalmouth_daily WHERE site_name != \'Vent (Deep)\' AND var=\'temp\';"
...
```
```{r dy_tempnorth}
fal.temp <- transmutefalmouth(fal, "temp")
temp.xts <- xts(fal.temp[,-1], order.by=fal.temp[,1])
names(temp.xts) <- c("AD",
                     "D",
                     "B",
                     "C",
...
dygraph(temp.xts[,c(-7,-8,-9)], main="2D-ACM Temperature (C), Northern Group", group="a") %>% 
...
```
```{r dy_tempsouth}
dygraph(temp.xts[,c(7,8,9)], main="2D-ACM Temperature (C), Southern Group", group="a") %>% 
    #dySeries("SC1", axis='y2') %>% 
    #dySeries("SC10", axis='y2') %>% 
    #dySeries("REV",axis='y2') %>%
