---
title: "FGS 2D-ACM Average Speed dygraphs"
...
sql <- "SELECT * FROM rfalmouth_daily WHERE site_name != \'Vent (Deep)\' AND var=\'aspd\';"
...
aspd.xts <- xts(fal.aspd[,-1], order.by=fal.aspd[,1])
names(aspd.xts) <- c("AD",
                     "D",
                     "B",
                     "C",
...
dygraph(aspd.xts[,c(-7,-8,-9)], main="2D-ACM Average Speed (cm/s), Northern Group", group="a") %>% 
...
```
```{r dy_aspdsouth}
dygraph(aspd.xts[,c(7,8,9)], main="2D-ACM Average Speed (cm/s), Southern Group", group="a") %>% 
    #dySeries("SC1", axis='y2') %>% 
    #dySeries("SC10", axis='y2') %>% 
    #dySeries("REV",axis='y2') %>%
