---
title: "FGS 2D-ACM Pressure dygraphs"
...
sql <- "SELECT * FROM rfalmouth_daily WHERE site_name != \'Vent (Deep)\' AND var=\'pres\';"
...
```{r dy_presnorth}
fal.pres <- transmutefalmouth(fal, "pres")
pres.xts <- xts(fal.pres[,-1], order.by=fal.pres[,1])
names(pres.xts) <- c("AD",
                     "D",
                     "B",
                     "C",
...
dygraph(pres.xts[,c(-7,-8,-9)], main="2D-ACM Pressure (dbar), Northern Group", group="a") %>% 
...
```{r dy_pressouth}
dygraph(pres.xts[,c(7,8,9)], main="2D-ACM Pressure (dbar), Southern Group", group="a") %>% 
    #dySeries("SC1", axis='y2') %>% 
    #dySeries("SC10", axis='y2') %>% 
    #dySeries("REV",axis='y2') %>%
